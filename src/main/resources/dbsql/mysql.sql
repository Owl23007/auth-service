...
CREATE TABLE `role`
(
    `id`          TINYINT UNSIGNED NOT NULL COMMENT '角色ID',
...
-- 初始化角色数据
INSERT INTO `role` (`id`, `name`, `zh_name`, `description`)
VALUES (0, 'USER', '普通用户', '普通注册用户'),
...
(
    ...
    `role`             TINYINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '用户角色ID，引用role表',
...
(
    ...
    `user_id`                INT(11)      NOT NULL COMMENT '用户ID',
    `aud`                    VARCHAR(100) NOT NULL COMMENT 'JWT受众（audience），当前为登录时的服务名',
  ...
  DEFAULT CHARSET = utf8mb4 COMMENT ='用户登录Token表';